namespace :daemon do
  ...
  task :get_id => :environment do
    ...
    ProductList.unblocked.select(:id).find_each{ |l| GetPaginationWorker.perform_async(l.id) }
  end
  ...
  task :update_price => :environment do
    ProductList.unblocked.select(:id).find_each{ |l| GetPaginationWorker.perform_async(l.id, "price") }
  end
